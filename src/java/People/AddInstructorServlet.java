...
public class AddInstructorServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            stu.insertDB(ID, FirstName, LastName, Street, City, State, Zip, Office, Email);
            out.println("Instructor added!");
            response.setHeader("Refresh", "2; URL=Admin.jsp");
        }
        catch(SQLException|ClassNotFoundException ex){
            out.println("Instructor not added. Another Instructor with the same ID might already be in database. Returning to Admin Page.");
